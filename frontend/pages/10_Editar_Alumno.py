...
def main():
    st.set_page_config(page_title="Alumno", page_icon="ÃƒÂƒÃ‚Â°ÃƒÂ…Ã‚Â¸ÃƒÂ‚Ã‚Â§ÃƒÂ¢Ã‚Â€Ã‚Â™", layout="wide")
    ensure_auth("ADMIN")
    render_sidebar_nav()
    render_topbar()
    ...
    with col2:
        if st.button("ÃƒÂƒÃ‚Â¢ÃƒÂ¢Ã‚Â€Ã‚Â ÃƒÂ‚Ã‚Â Volver"):
            go_back()
    ...
    if not create_mode:
        ...
        age_val = stu.get("age")
        # Mostrar matrÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â­cula activa actual
        try:
        ...
        if active:
            ...
            st.caption(f"SalÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n actual (activo): ID {active_classroom_id} ÃƒÂƒÃ‚Â‚ÃƒÂ‚Ã‚Â· Inicio: {active.get('start_date') or '-'}")
    else:
    ...
    with st.form("student_form", clear_on_submit=False):
        ...
        with col2:
            gender = st.selectbox(
                "GÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â©nero",
                options=["", "M", "F", "O"],
                index=["", "M", "F", "O"].index(stu.get("gender") or ""),
            )

        # MatrÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â­cula inicial (crear) o cambio de salÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n (editar)
        ...
        if create_mode:
            st.markdown("---")
            st.subheader("MatrÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â­cula inicial")
            if room_opts:
                ridx = st.selectbox("SalÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n*", options=list(range(len(room_opts))), format_func=lambda i: room_opts[i][1])
            ...
            # Cambio de salÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n opcional (solo uno activo)
            st.markdown("---")
            st.subheader("Cambio de salÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n (opcional)")
            if room_opts:
                ridx2 = st.selectbox(
                    "Nuevo salÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n (opcional)",
                ...
                if ridx2 != -1:
                    ...
                    start_d = st.date_input("Fecha de inicio en nuevo salÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n", value=date.today())
        ...
        if create_mode and not all([first_name, last_name, age, selected_room_id]):
            ...
            with st.spinner("Guardando..."):
                if create_mode:
                    ...
                    if ok:
                        # si se seleccionÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³ nuevo salÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n, validar y migrar matrÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â­cula
                        if selected_room_id:
                            if active_classroom_id and int(selected_room_id) == int(active_classroom_id):
                                st.warning("El salÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â³n seleccionado es el mismo que el actual. No se realizaron cambios de matrÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â­cula.")
                            else:
                                try:
                                    enrolls = api_get_enrollments(sid)
                                except Exception:
                                ...
                                for e in enrolls:
                                    if not e.get("end_date"):
                                        ...
                                        break
                                if active:
                                    # cerrar matrÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â­cula activa un dÃƒÂƒÃ‚ÂƒÃƒÂ‚Ã‚Â­a antes del nuevo inicio
